...
/* glmTranslate: Shift a model along three axis by a given amount.
*
* model      - initialized GLMmodel structure
* x - shiftfactor along x axis
* y - shiftfactor along y axis
* z - shiftfactor along z axis
*/
GLvoid
glmTranslate(GLMmodel* model, GLfloat x, GLfloat y, GLfloat z)
{
	int i;
...
/* glmRotation: rotation a model along given axis by a given amount.
*
* model      - initialized GLMmodel structure
* axis - specify which axis to rotate (0 = x-axis, 1 = y-axis 2 = z-axis)
* angle - rotation factor  
*/
GLvoid
glmRotation(GLMmodel* model, GLint axis, GLfloat angle)
{
	...
	cz = (maxz + minz) / 2.0;
	for (i = 1; i <= model->numvertices; i++) {
		vx = model->vertices[3 * i + 0] - cx;
		vy = model->vertices[3 * i + 1] - cy;
		vz = model->vertices[3 * i + 2] - cz;
		if (axis == 0){
