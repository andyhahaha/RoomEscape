...
/*void compute_quality_metrics(char * file1, char * file2, int block_size)
{
	...
	// Loading pictures
	img1 = imread(file1, CV_LOAD_IMAGE_GRAYSCALE);
	img2 = imread(file2, CV_LOAD_IMAGE_GRAYSCALE);
...
//img(num_next) * H
/*int choose_30max(int num_next, string path_src, Mat H, Mat img1, double ssim)
{
...
//img(num_now) * H
/*int choose_30max(int num_next, string path_src, Mat H, Mat img1_new, double ssim)
{
	int num_next_new = num_next;
	Vec3b pixel;
	pixel[0] = 0;
	...
	for (int i = num_next + 1; i < num_next + 31; i++)
	{
		...
		for (int i = 0; i < img1_new.rows; i++){
			for (int j = 0; j < img1_new.cols; j++){
				if (white_new.at<uchar>(i, j) == 0)
					compare.at<Vec3b>(i, j) = pixel;
			}
		}

		double ssim_compare = compute_quality_metrics(img1_new, compare, 120);
		if (ssim_compare > ssim)
		{
			num_next_new = i;
			ssim = ssim_compare;
...
/*void choose_image (string& room_name, int position_number, int vertical_angle)
{
	...
	int num_next = 401;
	int img_num = 1;
	double ssim, min=0.45, max=0.75;
	int state = 0;
	int count = 0, MaxSsim = 0;

	Mat img1, img2, white_new, img1_new, img2_new;

	//五度的矩陣(第二張圖乘上這個矩陣變五度前的圖)

	float h[] = { 0.9771534995932721, 0.0006864297562182481, 40.67085914342899,
			-0.007218203846980533, 0.988096140686749, 5.892864772165695,
			-1.147218731713906e-005, -8.278556386935633e-007, 1 };
	Mat H = Mat(3, 3, CV_32F, h).clone();
	...
	//先做一個白色的轉-5度的圖
	Mat white(img1.rows, img1.cols, CV_8U, Scalar(255));
	warpPerspective(white, white_new, H, Size(img1.cols, img1.rows));

	//第一張img1用white_new把邊邊塗黑
	Vec3b pixel;
	pixel[0] = 0;
	pixel[1] = 0;
	pixel[2] = 0;
	...
	while (1)
	{
		...
		warpPerspective(img2, img2_new, H, Size(img1.cols, img1.rows));
		cout << num_next << endl;

		ssim = compute_quality_metrics(img1, img2_new, 120);


		Mat img1_small, img2_small;
		resize(img1, img1_small, Size(int(img1.cols/ 4), int(img1.rows / 4)));
		resize(img2_new, img2_small, Size(int(img1.cols / 4), int(img1.rows / 4)));
		imshow("img1", img1_small);
		imshow("img2", img2_small);
		waitKey(10);
		...
		if (state < 2){
			if (ssim > max)
			{
				if (ssim < 0.9) num_next = choose_20max(num_next, path_src, H, img1, ssim);

				if (img_num > 300 && compute_quality_metrics(imread(path_dst + "1.jpg"), img2, 120) > 0.8) break;
			...
			else if (ssim < min)
			{
				...
				if (state < 2)
				{
					H = find_homography_matrix(path_dst, img_num);
					num_next = num_now + 1;
		...
		if (state == 2){
			cout << "choose_100max" << endl;
			num_next = num_now + 1;
			num_next = choose_100max(num_next, path_src, H, img1, ssim);
...
/*void choose_image(string& room_name, int position_number, int vertical_angle, int img_num1, int img_num2)
{
	...
	int num_next = 2;
	int img_num = 1;
	double ssim, ssim2, min = 0.45, max = 0.75;
	int state = 0;
	int count = 0, MaxSsim = 0;
	...
	Mat H = find_homography_matrix(path_src, img_num1, img_num2);
	...
	tm.stop();
	cout << "process time1=" << tm.getTimeMilli() << endl;
	...
	while (1)
	{
		...
		warpPerspective(img2, img2_new, H, Size(img1.cols, img1.rows));
		tm.stop();
		cout << "process time2=" << tm.getTimeMilli() << endl;
		...
		ssim = compute_quality_metrics(img1, img2_new, 120);
		cout << "ssim2 --- "<<endl;
		ssim2 = compute_quality_metrics(img1, img2, 120);	//將num_now和num_next的圖直接做比較，如果兩者太像就不要
		...
		if (ssim > max && ssim2<0.7)
		{
			max = 0.75;
			min = 0.45;
			if (ssim < 0.9) num_next = choose_30max(num_next, path_src, H, img1, ssim);

			img2 = imread(path_src + to_string(num_next) + ".jpg");
			if (img_num > 150 && compute_quality_metrics(imread(path_dst + "1.jpg"), img2, 120) > 0.7) break;
...
/*void choose_image(string& room_name, int position_number, int vertical_angle, int img_num1, int img_num2)
{
	...
	Mat img1, img2, img1_new, img2_new;
	TickMeter  tm;
	...
	img1 = imread(path_src + to_string(num_now) + ".jpg");
	imwrite(path_dst + to_string(img_num) + ".jpg", img1);
	warpPerspective(img1, img1_new, H, Size(img1.cols, img1.rows));

	//先做一個白色的轉-5度的圖
	Mat white;
	white = Mat::zeros(img1.rows, img1.cols, CV_8U) + 255.;
	warpPerspective(white, white_new, H, Size(img1.cols, img1.rows));
	...
	while (1)
	{
		...
		for (int i = 0; i < img2.rows; i++){
			for (int j = 0; j < img2.cols; j++){
				if (white_new.at<uchar>(i, j) == 0)
					img2.at<Vec3b>(i, j) = pixel;
			}
		}

		cout << "ssim2 --- " << endl;
		ssim = compute_quality_metrics(img1_new, img2, 120);

		//Mat img1_small, img2_small;
		//resize(img1_new, img1_small, Size(int(img1.cols / 4), int(img1.rows / 4)));
		//resize(img2, img2_small, Size(int(img1.cols / 4), int(img1.rows / 4)));
		//imshow("img1", img1_small);
		//imshow("img2", img2_small);
		//waitKey(10);
		...
		if (ssim > max && ssim2<0.8)
		{
			max = 0.75;
			min = 0.45;
			if (ssim < 0.9) num_next = choose_30max(num_next, path_src, H, img1_new, ssim);
